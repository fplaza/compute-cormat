...
class BlockingQueue
{
    private:
        std::queue<T> queue_;
        const boost::uint64_t max_size_;
        boost::mutex mutex_;
        boost::condition_variable not_empty_;
        boost::condition_variable not_full_;

    public:
        BlockingQueue<T>(const boost::uint64_t max_size);
        boost::uint64_t max_size() const;
        void enqueue(const T& val);
        T dequeue();
};

template <typename T>
BlockingQueue<T>::BlockingQueue(boost::uint64_t max_size)
: queue_(), max_size_(max_size), mutex_(), not_empty_(), not_full_()

{};

template <typename T>
boost::uint64_t BlockingQueue<T>::max_size() const
{
    return max_size_;
}
