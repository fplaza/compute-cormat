...
Parameters Parameters::parse(int argc, char* argv[])
{
    ...
    computation_parameters.add_options()
        ("num-threads,t", po::value<int>(&pars.num_threads)->default_value(omp_get_max_threads()), "Number of threads to launch in compute kernel")
        ("block-size,b", po::value<boost::uint64_t>(&Block::BLOCK_SIZE)->default_value(Block::DEFAULT_BLOCK_SIZE), "")
        ;

    output_parameters.add_options()
        ("cor-threshold,c", po::value<float>(&pars.cor_threshold)->default_value(-1.0f,"-1.0"), "Print only correlations above this threshold.")
        ("out-fmt,f", po::value<std::string>(&pars.output_format)->default_value("binary"), "Format in which correlations are printed ('binary' or 'text')")
    ...
    all_parameters.add(input_parameters).
        add(computation_parameters).
