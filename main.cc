...
int main(int argc, char *argv[])
{
	...
	try
	{
		...
		BlockingQueue<Tile*> tiles_to_recycle(2);
		BlockingQueue<Tile*> tiles_to_compute(2);
		BlockingQueue<Tile*> tiles_to_write(2);

		Scheduler scheduler(mat, tiles_to_recycle, tiles_to_compute);
		CorrelationsCalculator calculator(tiles_to_compute, tiles_to_write);
		std::auto_ptr<CorrelationsWriter>  writer =
			CorrelationsWriterFactory::create_writer(pars.output_format, pars.cors_file, pars.cor_threshold, tiles_to_write, tiles_to_recycle);
